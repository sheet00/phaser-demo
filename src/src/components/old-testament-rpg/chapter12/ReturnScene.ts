...
export default class ReturnScene extends ChronicleScene {
  ...
  protected startChapter() {
    ...
    this.talk([
      { speaker: 'ダニエルの祈りとキュロス王の勅令', body: '老預言者ダニエルの涙の祈りに応え、神はペルシア皇帝キュロスの心を動かされた。\n「天の神はエルサレムに神殿を建てるよう私に命じられた。捕囚の民は全員、故郷へ帰るがよい！」\n捕囚から70年、奇跡の帰還令（キュロスの勅令）がついに発布されたのだ！' },
      { speaker: '七十年ぶりの故郷エルサレムへ', body: '民は歓喜の涙を流し、砂漠を越えて先祖の約束の地エルサレムへと帰還した。\n指導者ゼルバベルのもとで主の祭壇を築き、ついに神殿（第二神殿）を再建した！' },
      { speaker: '崩れた城壁と敵の脅威', body: 'しかし神殿は建ったものの、都を守る「外壁（城壁）」は破壊されたままであった。\n周囲の敵民族から毎日のように脅迫や略奪を受け、都は無防備なまま放置されていた。' },
      { speaker: '総督ネヘミヤの赴任', body: 'ダニエルと同じくペルシア王宮で皇帝の最側近を務めていたユダヤ人「ネヘミヤ」は、\n故郷の惨状を聞いて断食祈祷。王の許しを得て総督として赴任し、城壁再建に立ち上がる！' },
    ]);
  }
