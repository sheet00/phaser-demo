...
export default function JacobJosephChapterGame() {
  const hostRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const host = hostRef.current;
    if (!host) return;
    const scene = new JacobJosephScene();
    const game = new Phaser.Game({ type: Phaser.AUTO, parent: host, width: window.innerWidth, height: window.innerHeight - 78, pixelArt: true, backgroundColor: '#101a35', scale: { mode: Phaser.Scale.RESIZE, autoCenter: Phaser.Scale.CENTER_BOTH }, input: { keyboard: { target: host } }, physics: { default: 'arcade', arcade: { gravity: { x: 0, y: 0 } } }, scene });
