...
Viteは `/api/dominion` をローカルのWorkerへ転送する。`http://localhost:5173/dominion` で「友達とネット対戦」を選び、作成者と参加者がそれぞれプレイヤー名を入力する。作成した招待リンクを別のブラウザーで開く。席トークンはブラウザーのローカルストレージに保存され、再読み込み後の復帰に使う。
